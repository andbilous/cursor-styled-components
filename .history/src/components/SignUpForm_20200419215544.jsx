...
class SignUpForm extends React.Component{
  ...
  validateFields=(type,value)=>{
    if(type==='firstName' || type ==='lastName' ){
      if(value.length<3){
        this[`${type}`].current.style.borderColor='red'
      }else{
        this[`${type}`].current.style.borderColor='green'
      }
    } 
    if(type ==='email'){
      if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)){
    ...
    <div class="form">
      <form class="login-form">
       ...
       <button onClick={()=>{
         ...
         localStorage.setItem('lastName',this.state.firstName)
         localStorage.setItem('password',this.state.firstName)
       }}>Sign Up</button>
